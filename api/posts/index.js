...
async function sendEmail(options) {
  try {
    ...
    console.log(`[Email] \u0110\xE3 g\u1EEDi email t\u1EDBi ${options.to}`);
  } catch (error) {
    console.error(`[Email] L\u1ED7i g\u1EEDi email t\u1EDBi ${options.to}:`, error);
  }
}
async function notifyNewPost(postId, authorEmail) {
  const postUrl = `${process.env.APP_URL || "http://localhost:8000"}/post/${postId}`;
  await sendEmail({
    to: authorEmail,
    subject: "[EduForum] B\xE0i vi\u1EBFt m\u1EDBi c\u1EE7a b\u1EA1n \u0111\xE3 \u0111\u01B0\u1EE3c \u0111\u0103ng th\xE0nh c\xF4ng!",
    html: `
      <div style="font-family: sans-serif; padding: 20px; line-height: 1.6;">
        <h2 style="color: #dc2626;">\u0110\u0103ng b\xE0i th\xE0nh c\xF4ng!</h2>
        <p>Xin ch\xE0o,</p>
        <p>B\xE0i vi\u1EBFt c\u1EE7a b\u1EA1n \u0111\xE3 \u0111\u01B0\u1EE3c t\u1EA3i l\xEAn di\u1EC5n \u0111\xE0n h\u1ECDc thu\u1EADt EduForum. B\u1EA1n c\xF3 th\u1EC3 xem chi ti\u1EBFt b\xE0i vi\u1EBFt t\u1EA1i \u0111\u01B0\u1EDDng d\u1EABn d\u01B0\u1EDBi \u0111\xE2y:</p>
        <p style="margin: 20px 0;">
          <a href="${postUrl}" style="background-color: #dc2626; color: white; padding: 10px 15px; text-decoration: none; border-radius: 4px; display: inline-block; font-weight: bold;">Xem b\xE0i vi\u1EBFt</a>
        </p>
        <p>C\u1EA3m \u01A1n b\u1EA1n \u0111\xE3 \u0111\xF3ng g\xF3p c\xE2u h\u1ECFi/ki\u1EBFn th\u1EE9c cho c\u1ED9ng \u0111\u1ED3ng.</p>
        <hr style="border: none; border-top: 1px solid #eee; margin: 20px 0;" />
        <p style="font-size: 12px; color: #888;">\u0110\xE2y l\xE0 email t\u1EF1 \u0111\u1ED9ng t\u1EEB EduForum. Vui l\xF2ng kh\xF4ng tr\u1EA3 l\u1EDDi email n\xE0y.</p>
      </div>
    `
  });
}
async function notifyNewReply(postId, recipientEmail) {
  const postUrl = `${process.env.APP_URL || "http://localhost:8000"}/post/${postId}`;
  await sendEmail({
    to: recipientEmail,
    subject: "[EduForum] C\xF3 ph\u1EA3n h\u1ED3i m\u1EDBi cho b\xE0i vi\u1EBFt c\u1EE7a b\u1EA1n!",
    html: `
      <div style="font-family: sans-serif; padding: 20px; line-height: 1.6;">
        <h2 style="color: #dc2626;">C\xF3 b\xECnh lu\u1EADn/ph\u1EA3n h\u1ED3i m\u1EDBi!</h2>
        <p>Xin ch\xE0o,</p>
        <p>M\u1ED9t c\xE2u tr\u1EA3 l\u1EDDi ho\u1EB7c b\xECnh lu\u1EADn m\u1EDBi v\u1EEBa \u0111\u01B0\u1EE3c \u0111\u0103ng t\u1EA3i tr\xEAn b\xE0i vi\u1EBFt li\xEAn quan c\u1EE7a b\u1EA1n t\u1EA1i EduForum.</p>
        <p style="margin: 20px 0;">
        ...
        <p>Tham gia th\u1EA3o lu\u1EADn ngay \u0111\u1EC3 c\xF9ng trao \u0111\u1ED5i h\u1ECDc thu\u1EADt.</p>
        <hr style="border: none; border-top: 1px solid #eee; margin: 20px 0;" />
        <p style="font-size: 12px; color: #888;">\u0110\xE2y l\xE0 email t\u1EF1 \u0111\u1ED9ng t\u1EEB EduForum. Vui l\xF2ng kh\xF4ng tr\u1EA3 l\u1EDDi email n\xE0y.</p>
      </div>
    `
...
var import_nodemailer, smtpHost, smtpPort, smtpUser, smtpPass, smtpFrom, transporter;
var init_email = __esm({
  "src/server/utils/email.ts"() {
    ...
    smtpFrom = process.env.SMTP_FROM || "noreply@eduforum.vn";
...
function validateCreatePostInput(data) {
  ...
  if (!data.title || !isValidTitle(data.title)) {
    errors.push({
      field: "title",
      message: "Ti\xEAu \u0111\u1EC1 ph\u1EA3i c\xF3 t\u1EEB 5 \u0111\u1EBFn 200 k\xFD t\u1EF1"
    });
  }
  if (!data.content || !isValidContent(data.content)) {
    errors.push({
      field: "content",
      message: "N\u1ED9i dung ph\u1EA3i c\xF3 t\u1EEB 10 \u0111\u1EBFn 10000 k\xFD t\u1EF1"
    });
  }
  if (!isValidTags(data.tags || [])) {
    errors.push({
      field: "tags",
      message: "Vui l\xF2ng ch\u1ECDn t\u1EEB 1 \u0111\u1EBFn 5 tag"
    });
  }
  return {
...
function formatTime(date) {
  const seconds = Math.floor(
    ((/* @__PURE__ */ new Date()).getTime() - new Date(date).getTime()) / 1e3
  );
...
function formatQuestion(q) {
  return {
    ...
    tags: q.questionTags ? q.questionTags.map((t) => t.name) : [],
    votes: q.votes,
...
async function handler(req, res) {
  ...
  if (req.method === "GET") {
    try {
      const dec = (v) => typeof v === "string" ? decodeURIComponent(v.replace(/\+/g, " ")) : "";
      ...
      const whereClause = { status: { [import_sequelize3.Op.ne]: "hidden" } };
      if (authorId) {
      ...
      if (q.trim()) {
        ...
        whereClause[import_sequelize3.Op.or] = [
          { title: { [import_sequelize3.Op.like]: keyword } },
          { content: { [import_sequelize3.Op.like]: keyword } }
        ];
      }
      ...
      if (dept.trim()) {
        authorInclude.where = { department: { [import_sequelize3.Op.like]: `%${dept.trim()}%` } };
  ...
  if (req.method === "POST") {
    try {
      ...
      if (Array.isArray(tags) && tags.length > 0) {
        ...
        await newQuestion.setQuestionTags(tagsInDb);
        await Promise.all(tagsInDb.map((t) => t.increment("count")));
      }
      await user.increment("posts");
      try {
        const { notifyNewPost: notifyNewPost2 } = await Promise.resolve().then(() => (init_email(), email_exports));
        await notifyNewPost2(newQuestion.id, user.email);
      } catch (err) {
