...
// src/.umi-production/api/posts/[id]/vote.ts
var vote_exports = {};
...
// src/api/posts/[id]/vote.ts
init_entities();
async function addReputation(author, voterId, delta) {
  ...
  if (req.method === "GET") {
    try {
      ...
      const postVotes = await VoteEntity.findAll({
        where: {
          [import_sequelize3.Op.or]: [
            { targetId: id, targetType: "question" },
            ...commentIds.length ? [{ targetType: "comment", targetId: { [import_sequelize3.Op.in]: commentIds } }] : []
          ]
        }
      });
      res.status(200).json({
        success: true,
        data: postVotes.map((v) => ({
          userId: v.userId,
          targetId: v.targetId,
...
var vote_default = async (req, res) => {
