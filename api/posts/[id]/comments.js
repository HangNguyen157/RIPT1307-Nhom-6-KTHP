...
async function handler(req, res) {
  ...
  if (req.method === "POST") {
    try {
      ...
      await user.save();
      try {
        const { notifyNewReply: notifyNewReply2 } = await Promise.resolve().then(() => (init_email(), email_exports));
        if (parentId) {
          ...
          if (parentComment && parentComment.author) {
            await notifyNewReply2(id, parentComment.author.email);
          }
        } else {
          const questionAuthor = await UserEntity.findByPk(question.authorId);
          if (questionAuthor) {
            await notifyNewReply2(id, questionAuthor.email);
          }
        }
      } catch (err) {
