...
// src/.umi-production/api/posts/[id]/comments.ts
...
function validateCommentInput(data) {
  const errors = [];
  if (!data.content || data.content.trim().length < 2 || data.content.trim().length > 5e3) {
    errors.push({
      field: "content",
      message: "B\xECnh lu\u1EADn ph\u1EA3i c\xF3 t\u1EEB 2 \u0111\u1EBFn 5000 k\xFD t\u1EF1"
    });
  }
  return {
...
async function handler(req, res) {
  ...
  if (req.method === "POST") {
    try {
      ...
      if (!auth) {
        res.status(401).json({ success: false, message: "Vui l\xF2ng \u0111\u0103ng nh\u1EADp \u0111\u1EC3 b\xECnh lu\u1EADn" });
        return;
      }
  ...
  if (req.method === "PUT") {
    try {
      ...
      if (!comment || comment.questionId !== id) {
        res.status(404).json({ success: false, message: "B\xECnh lu\u1EADn kh\xF4ng thu\u1ED9c b\xE0i vi\u1EBFt n\xE0y" });
        return;
      }
      if (isBest) {
...
var comments_default = async (req, res) => {
